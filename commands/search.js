...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (!name) return interaction.reply({ content: lang.msg73, flags: 64 }).catch(e => { })
 let res
try {
      ...
      interaction.reply(code).then(async Message => {
        ...
        collector.on('collect', async (button) => {
          switch (button.customId) {
            case 'cancel': {
              ...
              await interaction.editReply({ embeds: [embed], components: [] }).then(() => {
                // Auto delete cancel message after 5 seconds
                setTimeout(async () => {
                  try {
            ...
            default: {
              ...
              await interaction.editReply({ embeds: [embed], components: [] }).then(() => {
                // Auto delete selected song message after 10 seconds
                setTimeout(async () => {
                  try {
                    await interaction.deleteReply().catch(() => {});
                  } catch (e) {}
        ...
        collector.on('end', (msg, reason) => {
          ...
          if (reason === 'time') {
            ...
            return interaction.editReply({ embeds: [embed], components: [] }).then(() => {
              // Auto delete timeout message after 10 seconds
              setTimeout(async () => {
                try {
                  await interaction.deleteReply().catch(() => {});
                } catch (e) {}
