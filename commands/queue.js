  ...
  run: async (client, interaction) => {
    ...
    try {
     ...
     const queue = client.player.getQueue(interaction.guild.id);
      ...
      if (!queue.songs[0]) return interaction.reply({ content: lang.msg63, flags: 64 }).catch(e => { })
      ...
      const generateEmbed = async (start) => {
        ...
        if (!current || !current?.length > 0) return interaction.reply({ content: lang.msg63, flags: 64 }).catch(e => { })
