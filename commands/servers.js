...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if(!client.config.ownerID.includes(interaction?.user?.id)) return interaction.reply({ content: "You don't have permission to use this command.", flags: 64 }).catch(e => { });
...
if(config.shardManager.shardStatus == true){
            const getServer = async (guildID) => {
             ...
             if (!guild) return interaction.reply({ content: "I'm not in that server.", flags: 64 }).catch(e => { });
