...
if (!name) return interaction.reply({ content: lang.msg91, flags: 64 }).catch(e => { })
...
return interaction.reply({ content: lang.msg92, flags: 64 }).catch(e => { })
}
}
}

if (userplaylist?.playlist?.length >= client.config.playlistSettings.maxPlaylist) return interaction.reply({ content: lang.msg93, flags: 64 }).catch(e => { })
...
if (!playlist?.playlist?.filter(p => p.name === name).length > 0) return interaction.reply({ content: lang.msg96, flags: 64 }).catch(e => { })
...
if (stp === "add-music") {
let name = interaction.options.getString('name')
if (!name) return interaction.reply({ content: lang.msg99, flags: 64 }).catch(e => { })
let playlist_name = interaction.options.getString('playlist-name')
if (!playlist_name) return interaction.reply({ content: lang.msg100, flags: 64 }).catch(e => { })

const playlist = await db.playlist.findOne({ userID: interaction.user.id }).catch(e => { })
if (!playlist?.playlist?.filter(p => p.name === playlist_name).length > 0) return interaction.reply({ content: lang.msg96, flags: 64 }).catch(e => { })

let max_music = client.config.playlistSettings.maxMusic
if (playlist?.musics?.filter(m => m.playlist_name === playlist_name).length > max_music) return interaction.reply({ content: lang.msg101.replace("{max_music}", max_music), flags: 64 }).catch(e => { })
let res 
try{
...
return interaction.reply({ content: lang.msg74, flags: 64 }).catch(e => { })
}
if(!res || !res.length || !res.length > 1) return interaction.reply({ content: lang.msg74, flags: 64 }).catch(e => { })
...
if (music_filter?.length > 0) return interaction.editReply({ content: lang.msg104, flags: 64 }).catch(e => { })
...
if (!name) return interaction.reply({ content: lang.msg99, flags: 64 }).catch(e => { })
let playlist_name = interaction.options.getString('playlist-name')
if (!playlist_name) return interaction.reply({ content: lang.msg106, flags: 64 }).catch(e => { })
...
if (stp === "list") {
let name = interaction.options.getString('name')
if (!name) return interaction.reply({ content: lang.msg110, flags: 64 }).catch(e => { })

let trackl

const playlist = await db.playlist.find().catch(e => { })
if (!playlist?.length > 0) return interaction.reply({ content: lang.msg96, flags: 64 }).catch(e => { })
...
if (!trackl?.length > 0) return interaction.reply({ content: lang.msg111, flags: 64 }).catch(e => { })

} else {
arr++
if (arr === playlist.length) {
return interaction.reply({ content: lang.msg58, flags: 64 }).catch(e => { })
}
}
}
...
if (!current || !current?.length > 0) return interaction.reply({ content: lang.msg111, flags: 64 }).catch(e => { })
...
collector.stop()
return button.reply({ content: `${lang.msg68}`, flags: 64 }).catch(e => { })
} else {
...
if (stp === "lists") {
const playlist = await db?.playlist?.findOne({ userID: interaction.user.id }).catch(e => { })
if (!playlist?.playlist?.length > 0) return interaction.reply({ content: lang.msg117, flags: 64 }).catch(e => { })
...
if (!playlists?.length > 0) return interaction.reply({ content: lang.msg114, flags: 64 }).catch(e => { })
...
if (!trackl?.length > 0) return interaction.reply({ content: lang.msg114, flags: 64 }).catch(e => { })
...
let sayı = page === 1 ? 1 : page * kaçtane - kaçtane + 1
const current = trackl.slice(start, start + kaçtane)
if (!current || !current?.length > 0) return interaction.reply({ content: lang.msg114, flags: 64 }).catch(e => { })
return new EmbedBuilder()
