...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (number) {
        if (!queue.songs.length > number) return interaction.reply({ content: lang.msg82, flags: 64 }).catch(e => { })
        if (isNaN(number)) return interaction.reply({ content: lang.msg130, flags: 64 }).catch(e => { })
        if (1 > number) return interaction.reply({ content: lang.msg130, flags: 64 }).catch(e => { })
        ...
        return interaction.reply({ content: lang.msg63, flags: 64 }).catch(e => { })
      }
      } else {
        try {
          let old = queue.songs[0];
